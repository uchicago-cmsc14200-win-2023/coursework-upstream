...
def do_test_contains(words, sought, result):
    ...
    for word in words:
        to.insert(word)
    assert to.contains(sought)==result

def do_test_completions(words, prefix, result):
    to = TrieOrthographer()
    for word in words:
        to.insert(word)
    assert sorted(to.completions(prefix)) == sorted(result)

def do_test_num_completions(words, prefix, result):
    to = TrieOrthographer()
    for word in words:
        to.insert(word)
    assert to.num_completions(prefix)==result

def do_test_all_words(words, result):
    to = TrieOrthographer()
    for word in words:
        to.insert(word)
    assert sorted(to.all_words()) == sorted(result)

def do_test_num_words(words, result):
    to = TrieOrthographer()
    for word in words:
        to.insert(word)
    assert to.num_words() == result
